...
cria_as_coisas = function(vrv,mn,mx,spr,co = cor,vsmx = 5,hsmx = 5,col=1,ixmn = 1.5,ixmx = 3,iymn = 1.5,iymx = 3,vsmn = 5,hsmn = 5,sprin = 0,angle = 0){
    ...
    if (variable_instance_get(id,vrv) = 1){
    	...
    	for (var b=0;b<r;b++){
    		...
    		for (var m=0;m<array_length(particulas[0]);m++){
    			...
    			var iy =random_range(iyms[0],iyms[1])
    			var hs = random_range(velm[1],vels[1])
    			var vs = random_range(velm[0],vels[0])
    			var itens = [0,d,ix,iy,co,hs,vs,xx,yy,spr,col,spri,angle]
