...
if ((place_meeting(x+_x,y,obj_colisores_pai) or place_meeting(x,y+_y,obj_colisores_pai)) and instance_place(x,y,obj_colisores_pai) != pai ){
	...
	for (var b=0;b<array_length(colisao);b++){
		...
		for (var m = 0;m<array_length(colisao[b]);m++){
			...
			if (col){
				...
				var vels  = [5							,5							,global.tiros_velo[i]		,5							]
				var velm  = [1							,1							,global.tiros_velo[i]/4		,1							]
                seta_part("cria_parede",x-_x,y-_y,[dano,dano_max],spr_particula_parede,cores[b],direction,[vels[b],vels[b]],[2,2.5],distc[b],[velm[b],velm[b]])
