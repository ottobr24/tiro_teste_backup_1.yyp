	...
	if (i<array_length(global.armas_mods[0])){
		...
		for (var m=array_length(mods)-1;m>=0;m--){
			...
			if (array_length(global.armas_modn[i][m])>0 and is_array(global.armas_mode[i][m][mods[m]])){
				...
				if (global.armas_mode[i][m][mods[m]][3] = 12) pente_ext		= m+1
				if (global.armas_mode[i][m][mods[m]][3] = 4	) silenciador	= m+1
			...
			if (sprreal){
				...
				var cano_calor = 255 - 255 / cano_quente_max * cano_quente
				
				var mod_cor = silenciador - 1 = m ? make_colour_rgb(255,cano_calor,cano_calor) : c_white
				
				draw_sprite_ext(mod_spr,mod_ii,mod_x,mod_y,image_xscale,image_yscale,image_angle,mod_cor,image_alpha)
...
atira = function(){
	...
	if (!modo){ //atira normal
		...
		#region Tiros normais
		
		if (atn or raj){
		
			if (tiro>0){
				
				#region Vars e rajada
				...
				#region Tiro de vdd, fogo e calor
				...
				var fog_qut_ix  = cano_quente / 250
				var fog_qut_tem = cano_quente / 100
				...
				if (fog_chan > 50){
					...
					fogo_tempo	= random_range(6,15) + fog_qut_tem
					fogo_ii		= irandom_range(0,sprite_get_number(fogo))
					fogo_ix		= tiro_vel/10 + fog_qut_ix
				
				}
				
				cano_quente = clamp(cano_quente,0,cano_quente_max)
				...
				if (part_tiro){ 
					...
					seta_part("cria_tiro",tx,ty,[part_tiro,part_tiro],tiro_spr,c_white,dirp,[tiiv,tiiv],[image_xscale*1,image_xscale*1.25],[1,1.25],45,[tiiv/2,tiiv/2],0,0)
				...
				#region Sem municao e fazendo barulho
				...
				if (i<array_length(global.armas_sons) and array_length(global.armas_sons[i])>0 and (asset_get_type(global.armas_sons[i][0]) == asset_sound or is_array(global.armas_sons[i][0]))){ 
		...
		if (prs_tec and tirg>0){
			...
			global.shakex += lengthdir_x(shak,direction) * 2 * global.shakes[global.configs[1][0]] 
			global.shakey += lengthdir_y(shak,direction) * 2 * global.shakes[global.configs[1][0]] 
...
preparando = function(){
	...
	if (man or sem){
		...
		if ((cock = 1 or man) and part_cock){ 
			...
			seta_part("cria_tiro",tx,ty,[part_cock,part_cock],tiro_spr,c_white,dirp,[tiiv,tiiv],[image_xscale*1,image_xscale*1.25],[1,1.25],45,[tiiv/2,tiiv/2],0,0)
			...
			if (tiro>0){
				...
				if (poc and !ejets){ 
					...
					if (qtd){
						...
						seta_part("cria_tiro",tx,ty,[qtd,qtd],tiro_spr,c_white,dirp,[tiiv,tiiv],[image_xscale*1,image_xscale*1.25],[1,1.25],45,[tiiv/2,tiiv/2],0,0)
	...
	if (rec){
		...
		if (rec_tec and !recarregando){ 
			...
			if (tiro = municao and global.armas_rext[i]=1){
				...
				tiro--
				seta_part("cria_tiro",x,y,[1,1],tiro_spr,c_white,dirt,[tiro_v,tiro_v],[image_xscale*1,image_xscale*1.25],[1,1.25],45,[tiro_v/2,tiro_v/2],0,0)
	...
	if (recarregando){
		...
		if (recarregando_timer<=0){
			...
			if (munp){
				...
				seta_part("cria_tiro",x,y,[munp,munp],tiro_spr,c_white,dirt,[tiro_v,tiro_v],[image_xscale*1,image_xscale*1.25],[1,1.25],45,[tiro_v/2,tiro_v/2],0,0)
...
inclinando_arma = function(){
	...
	if (abs(tec)){
		...
		with(pai){
			...
			var dir = other.direction - (tec * 90)
